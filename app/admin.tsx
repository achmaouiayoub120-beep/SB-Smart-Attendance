...
import type { Student, Professor, Module } from '@/src/types';
...
export default function AdminScreen() {
  ...
  const openEditModal = (item: Student | Professor | Module) => {
    ...
    if ('studentId' in item) {
      ...
      setFormDepartment(student.department || 'Informatique');
      setFormStudentId(student.studentId || '');
    } else if ('totalSessions' in item && !('code' in item)) {
      // Professor
      const prof = item as Professor;
      setFormFirstName(prof.firstName || '');
      setFormLastName(prof.lastName || '');
      setFormEmail(prof.email || '');
      setFormDepartment(prof.department || 'Informatique');
    } else if ('code' in item) {
      // Module
      const mod = item as Module;
      setFormModuleName(mod.name || '');
      setFormModuleCode(mod.code || '');
      setFormModuleProfessor(mod.professorId || '');
    }
      ...
      if (modalType === 'student') {
        ...
        if (editingItem && 'code' in editingItem) {
          await updateModule(editingItem.id, {
            name: formModuleName,
            code: formModuleCode,
            professorId: formModuleProfessor || '',
